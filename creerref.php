...
//session_start();
	...
	if($_SESSION['statut'] <4){
		echo("<h2>Votre statut ne vous autorise pas à créer une référence </h2>");
...
<html>
    ...
    <body>      
    	<?php
			echo '<h1>Creer une nouvelle référence pour : '.$_SESSION['stockname']. '</h1>';
	    ...
	    <form action="<?php echo($_SERVER['PHP_SELF']); ?>" method="post" id="chg">
            ...
            <input type="text" id="reference" name="reference" size='65' maxlength='65' value="<?php if (isset($_POST['reference'])){echo $_POST['reference'];} ?>"</td></br>
 			...
 			<?php include "categoriesliees.php" ?></br>

 			<?php include "destinationsliees.php" ?></br>
...
<!--
            ...
            <label for="dateha">Date d'achat (+) :</label>
            <input type="text" id="dateha" name="dateha" size='65' maxlength='65' value="<?php if (isset($_POST['dateha'])){echo $_POST['dateha'];} ?>"</td></br>
-->
            <label for="bouton">Validation :</label>
            <input type="submit" name="ok" id="ok" value="Valider cette référence" ></td>
        </form>
	</body>
</html>